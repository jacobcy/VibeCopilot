...
from src.roadmap.validator.yaml_validator_schema import SECTION_NAMES, VALID_PRIORITY, VALID_STATUS
    ...
    def validate_section(
        ...
        for field in fields["required"]:
            if field not in section:
                ...
                if field == "title":
                    ...
                    fixed_section[field] = f"这是一个示例{SectionValidator.get_section_name(section_type)}"
        ...
        if "status" in section and section_type in VALID_STATUS:
            if section["status"] not in VALID_STATUS[section_type]:
                messages.append(f"⚠️ 警告: {section_type} #{index+1} 状态 '{section['status']}' 无效，" f"有效值: {', '.join(VALID_STATUS[section_type])}")
        ...
        if section_type == "task" and "priority" in section:
            if section["priority"] not in VALID_PRIORITY:
                messages.append(f"⚠️ 警告: 任务 #{index+1} 优先级 '{section['priority']}' 无效，" f"有效值: {', '.join(VALID_PRIORITY)}")
        ...
        if "progress" in section:
            try:
                ...
                messages.append(f"⚠️ 警告: {section_type} #{index+1} 进度值 '{section['progress']}' 不是有效数字")
