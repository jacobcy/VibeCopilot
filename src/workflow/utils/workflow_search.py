...
# Add missing import for session factory
...
def get_workflow_fuzzy(identifier: str) -> Optional[Dict[str, Any]]:
    """
    通过模糊匹配查找工作流，支持ID、名称或部分匹配 (主要查询数据库)
    ...
    """
    logger.debug(f"开始查找工作流: '{identifier}' (using DB session context)")
    ...
    with get_session_factory()() as session:
        ...
        if is_id:
            ...
            workflow = get_workflow_by_id(identifier, session=session)
            if workflow:
                logger.info(f"通过ID精确匹配找到工作流: {workflow.get('name', '未命名')} (ID: {workflow.get('id')})")
                return workflow

        # 尝试精确匹配名称
        workflow = get_workflow_by_name(identifier, session=session)
        if workflow:
        ...
        # 使用传入的 session 调用 list_workflows
        workflows = list_workflows(session=session)
    ...
    # If with block finishes without returning, means not found
    return None
