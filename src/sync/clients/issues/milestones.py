...
class GitHubIssueMilestonesClient(GitHubClientBase):
    ...
    def create_milestone(
        ...
        self.logger.error(f"向 {endpoint} 发送 POST 请求以创建里程碑，payload: {data}")
        try:
            return self.post(endpoint, payload=data)
        except requests.exceptions.HTTPError as e:
            if e.response is not None:
                self.logger.error(f"HTTPError 创建里程碑 '{title}' in {owner}/{repo}. Status: {e.response.status_code}. URL: {e.response.url}")
                try:
                    error_details = e.response.json()
                    self.logger.error(f"GitHub API Error Details for creating milestone '{title}': {error_details}")
                except ValueError:
                    self.logger.error(f"GitHub API Response (non-JSON) for creating milestone '{title}': {e.response.text}")
            else:
                self.logger.error(f"HTTPError 创建里程碑 '{title}' in {owner}/{repo} (no response object): {e}")
            raise
        except Exception as e:
            self.logger.error(f"创建里程碑 '{title}' in {owner}/{repo} 时发生一般错误: {e}", exc_info=True)
            raise
