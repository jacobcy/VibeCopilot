...
class GitHubIssuesClient(GitHubClientBase):
    ...
    def update_issue(
        ...
        """更新问题."""
        self.logger.debug(
            f"Preparing to update Issue #{issue_number} in {owner}/{repo} with data: title={title}, body={body}, state={state}, assignees={assignees}, milestone={milestone}, labels={labels}"
        )
        try:
            # 调用核心客户端执行更新
            ...
            if e.response is not None:
                self.logger.error(f"HTTPError更新Issue #{issue_number} in {owner}/{repo}. Status: {e.response.status_code}. URL: {e.response.url}")
                if e.response.status_code == 422:
                    try:
                        error_details = e.response.json()
                        self.logger.error(f"GitHub API 422 Error Details for Issue #{issue_number}: {error_details}")
                    except ValueError:
                        self.logger.error(f"GitHub API 422 Response (non-JSON) for Issue #{issue_number}: {e.response.text}")
            else:
                self.logger.error(f"HTTPError更新Issue #{issue_number} in {owner}/{repo} (no response object): {e}")
            # 可以选择在这里重新抛出异常，或者根据需要返回 None/False
            # raise # 如果希望上层也能捕获到这个 HTTPError
            return None  # 或者返回None表示失败
        except Exception as e:
            self.logger.error(f"更新Issue #{issue_number} in {owner}/{repo} 时发生一般错误: {e}", exc_info=True)
            return None
