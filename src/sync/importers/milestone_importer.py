        ...
        for milestone_data in yaml_data["milestones"]:
            try:
                ...
                if "tasks" in milestone_data and isinstance(milestone_data["tasks"], list):
                    # 使用 session_scope 为任务处理创建会话
                    with session_scope() as session:
                        # 为此里程碑获取或创建隐式Story ID
                        implicit_story_id_for_milestone_tasks = self._get_or_create_implicit_story_for_milestone(
                            session, milestone_id, milestone_title, roadmap_id
                        )

                        if implicit_story_id_for_milestone_tasks:
                            task_importer.import_tasks(
                                tasks_data=milestone_data["tasks"],
                                milestone_id=milestone_id,
                                roadmap_id=roadmap_id,
                                import_stats=import_stats,
                                story_id=implicit_story_id_for_milestone_tasks,  # 关键修改，提供story_id
                            )
                        else:
                            # 记录错误：未能为里程碑任务创建/获取隐式Story，这些任务将无法导入
                            num_tasks_skipped = len(milestone_data["tasks"])
                            self.log_error(f"由于未能创建/获取里程碑 '{milestone_title}' 的隐式Story，其下的 {num_tasks_skipped} 个任务无法导入。")
                            if "tasks" in import_stats:
                                import_stats["tasks"]["failed"] += num_tasks_skipped

            except Exception as e:
                error_msg = f"导入里程碑失败: {milestone_data.get('title', 'Unknown')}"
                self.handle_import_error(error_msg, e, import_stats, "milestones")

    def _get_or_create_implicit_story_for_milestone(
        self, session: Session, local_milestone_id: str, local_milestone_title: str, local_roadmap_id: str
    ) -> Optional[str]:
        """
        ...
        """
        # 使用RoadmapService中的共享方法
        return self.service.get_or_create_implicit_story_for_milestone(session, local_milestone_id, local_milestone_title, local_roadmap_id)
