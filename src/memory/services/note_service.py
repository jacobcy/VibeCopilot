...
class NoteService:
    ...
    def create_note(self, content: str, title: str, folder: str, tags: Optional[str] = None) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        # 首先创建到远程Basic Memory，利用note_utils中的create_note方法
        ...
        if success:
            try:
                ...
                self.db_service.memory_item_repo.create_item(
                    title=title,
                    content=content,
                    folder=folder,
                    tags=tags,
                    permalink=permalink
                    # sync_status 默认在repo中处理为 NOT_SYNCED
                )
    ...
    def read_note(self, path: str, use_local: bool = True) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if success and use_local:
            try:
                ...
                # 检查是否已存在
                item = repo.get_by_permalink(metadata.get("permalink", path))
    ...
    def update_note(self, path: str, content: str, tags: Optional[str] = None) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        if success:
            try:
                ...
                item = repo.get_by_permalink(permalink)
    ...
    def search_notes(self, query: str, use_local: bool = True) -> Tuple[bool, str, List[Dict[str, Any]]]:
        ...
        if use_local:
            try:
                ...
                # 从本地数据库搜索
                items = repo.search_items(query=query)
    ...
    def sync_with_remote(self, notes: List[Dict[str, Any]]) -> Tuple[bool, str, Dict[str, Any]]:
        ...
        try:
            ...
            for note_data in notes:
                ...
                try:
                    # 让 Repository 处理同步逻辑
                    item, is_new = repo.sync_item_from_remote(note_data)
                    if is_new:
                        created_count += 1
                    else:
                        updated_count += 1
