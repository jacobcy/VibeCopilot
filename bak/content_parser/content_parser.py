...
from adapters.content_parser.openai import OpenAIDocumentParser, OpenAIGenericParser, OpenAIRuleParser
...
class ContentParser:
    ...
    def parse_content(self, content: str, content_type: str = "generic", context: str = "") -> Dict[str, Any]:
        """解析文本内容
    ...
    def parse_directory(self, directory: str, pattern: str = "*.md", content_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """解析目录下的所有匹配文件
    ...
    def _infer_content_type(self, file_path: str) -> str:
        ...
        # 规则类型检测
        if any(keyword in path_lower for keyword in ["-rule", "/rule", "_rule", "-rules", "/rules", "_rules"]):
            return "rule"

        # 文档类型检测
        if any(keyword in path_lower for keyword in ["/doc", "-doc", "_doc", "/docs", "-docs", "_docs"]):
            return "document"
