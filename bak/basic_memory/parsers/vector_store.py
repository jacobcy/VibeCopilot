...
class VectorManager:
    ...
    def save_vector_store(self, vector_store: FAISS, name: str = "index") -> str:
        ...
        metadata = {
            ...
            "model": self.model,
            "created_at": os.path.getmtime(save_path / "index.faiss") if (save_path / "index.faiss").exists() else None,
        }
    ...
    def load_vector_store(self, path: Optional[str] = None) -> FAISS:
        ...
        # 加载向量存储
        vector_store = FAISS.load_local(str(load_path), self.embeddings, allow_dangerous_deserialization=True)
        ...
        try:
            ...
            if metadata_path.exists():
                ...
                print(f"向量存储信息: {metadata.get('chunks_count', '未知')} 个块, " f"模型: {metadata.get('model', '未知')}")
