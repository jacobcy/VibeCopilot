...
def check_environment():
    ...
    # 检查OpenAI API Key
    if not env_vars["OPENAI_API_KEY"] and (
        not os.environ.get("VIBE_RULE_PARSER") or os.environ.get("VIBE_RULE_PARSER") == "openai"
    ):
...
def main():
    ...
    if args.command == "parse":
        ...
        try:
            ...
            # 输出结果
            print(
                json.dumps(conflict_result, ensure_ascii=False, indent=2 if args.pretty else None)
            )
            ...
            if conflict_result.get("has_conflict"):
                ...
                print(
                    f"冲突描述: {conflict_result.get('conflict_description', '无详细描述')}", file=sys.stderr
                )
            else:
